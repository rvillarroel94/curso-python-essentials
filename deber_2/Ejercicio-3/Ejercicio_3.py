...
def main():
    # Seteamos la variable del control del while para poder saber cuando terminar el bucle infinito
    running = True
    while running:
        # Se imprime en consola las opciones disponibles para el usuario
        print('Elija una opción: ')
        print('1) Demostración del cálculo de máximos y mínimos en diccionarios.')
        print('2) Salir.')
        # Se pide al usuario que ingrese la opcion que desee
        option = input('Que opción desea: ')
        
        if(option=='1'):
            # En caso de que el usuario selecione la opcion 1 el programa se ejecuta
            getMaxMin()
        elif(option=='2'):
            # En caso de que el usuario selecione la opcion 2 el programa se termina
            running=False
        else:
            # En caso de que el usuario no ingrese una opcion valida el programa vuelve a solicitar una opción valida
            print('El valor ingresado no es correcto. Vuelva a intentarlo...')
